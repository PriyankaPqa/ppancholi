/* eslint-disable */
// This code is based on Odata-query version 7.0.6 https://github.com/techniq/odata-query
// We need to use this updated version for solving the issue of duplicate query - line 327
...
params.$orderby = buildOrderBy(orderBy);
  ...
  if (aliases.length > 0) {
    Object.assign(params, aliases.reduce(
(acc, alias) => Object.assign(acc, { [`@${alias.name}`]: handleValue(alias.value) }),
       {},
));
  }
...
function buildFilter<T>(filters: Filter<T> = {}, aliases: Alias[] = [], propPrefix = ''): string {
 ...
 result.push(collectionClause);
      ...
      // temporary fix for the issue of duplicate query string https://github.com/techniq/odata-query/issues/100
      filterExpr = Array.from(new Set(filtersArray)).join(' and ');
    } /* else {
