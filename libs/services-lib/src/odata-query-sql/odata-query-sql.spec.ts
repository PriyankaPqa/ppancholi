/* eslint-disable */
import buildQuerySql, { Expand, OrderBy, alias, json, ITEM_ROOT, decimal, TypedFilter } from './odata-query-sql';
...
describe('filter', () => {
  describe('comparison operators', () => {
    it('should handle basic filter without operator', () => {
      ...
      const actual = buildQuerySql({ filter });
    ...
    it('should allow passing filter as an array of typed filters and strings', () => {
      ...
      const expected = "?$filter=Height eq 1 and Width lt 5 and startswith(Name, 'R')";
      const actual = buildQuerySql({ filter });
      expect(actual).toEqual(expected);
    });
    ...
    it('should allow "in" operator in combination with other conditions', () => {
      ...
      const expected = "?$filter=contains(SomeNames,'Bob') and SomeNames in ('Peter Newman','Bob Ross','Bobby Parker','Mike Bobson')";
      const actual = buildQuerySql({ filter });
      expect(actual).toEqual(expected);
    });
  ...
  describe('logical operators', () => {
    ...
    it('should handle simple logical operators (not) as an array', () => {
      ...
      const expected = "?$filter=not ((startswith(FooProp,'foo')) and (startswith(BarProp,'bar')) and (FizProp in (1,2,3)))";
      const actual = buildQuerySql({ filter });
      expect(actual).toEqual(expected);
    });
    ...
    it('should ignore implied logical operator with undefined filters', () => {
      const filter = [undefined] as string[];
      const expected = '';
      const actual = buildQuerySql({ filter });
      expect(actual).toEqual(expected);
    });
    ...
    it('should omit/ignore undefined filters', () => {
      const filter = { and: [undefined] as string[] };
      const expected = '';
      const actual = buildQuerySql({ filter });
      expect(actual).toEqual(expected);
    });
    ...
    it('should omit/ignore null filters', () => {
      const filter = { and: [null] as string[] };
      const expected = '';
      const actual = buildQuerySql({ filter });
      expect(actual).toEqual(expected);
    });
    ...
    it('should handle implied logical operator on a single property using alises', () => {
      ...
      let expected = '?$filter=DateProp ge @start and DateProp le @end&@start=2017-01-01T00:00:00.000Z&@end=2017-03-01T00:00:00.000Z';
      let actual = buildQuerySql({ filter });
      expect(actual).toEqual(expected);
      end.value = new Date(Date.UTC(2017, 5, 1));
      expected = '?$filter=DateProp ge @start and DateProp le @end&@start=2017-01-01T00:00:00.000Z&@end=2017-06-01T00:00:00.000Z';
      actual = buildQuerySql({ filter });
      expect(actual).toEqual(expected);
    });
        ...
        or: [
          {
            ...
            'Prop3/NestedProp3': { ne: null as string },
          },
          {
    ...
    it('should handle nested logical operators and nested properties using "/" selectors', () => {
      ...
      const filter = {
        ...
        or: [
          {
            'Prop2/NestedProp2/DeeplyNestedProp2': 2,
            'Prop2/NestedProp3': { ne: null as string },
          },
          {
            'Prop4/NestedProp4/DeeplyNestedProp4': 4,
          },
  ...
  describe('collection operators', () => {
    ...
    it('should handle collection operator with object as implied `and`', () => {
      ...
      const expected = '?$filter=Tasks/any(tasks:tasks/AssignedGroupId eq 1234 and tasks/StatusId eq 300)';
      const actual = buildQuerySql({ filter });
      expect(actual).toEqual(expected);
    });
    ...
    it('should handle collection operator with nested collection operator', () => {
      ...
      const expected = '?$filter=Tasks/any(tasks:tasks/SubTasks/any(subtasks:subtasks/AssignedGroupId eq 1234 and subtasks/StatusId eq 300))';
      const actual = buildQuerySql({ filter });
      expect(actual).toEqual(expected);
    });
    ...
    it('should handle collection operator with nested collection operators', () => {
      ...
      const expected = '?$filter=Tasks/any(tasks:tasks/SubTasks/any(subtasks:subtasks/AssignedGroupId eq 1234 and subtasks/StatusId eq 300) and tasks/OtherTasks/all(othertasks:othertasks/StatusId eq 122 and othertasks/AssignedGroupId eq 2345))';
      const actual = buildQuerySql({ filter });
      expect(actual).toEqual(expected);
    });
    ...
    it('should handle collection operator with object specifying operator', () => {
      ...
      const expected = '?$filter=Tasks/any(tasks:((tasks/AssignedGroupId eq 1234) or (tasks/StatusId eq 300)))';
      const actual = buildQuerySql({ filter });
      expect(actual).toEqual(expected);
    });
    ...
    it('should handle collection operator with nested property', () => {
      ...
      const expected = "?$filter=Tasks/any(tasks:tasks/CreatedBy/Name eq 'Sean Lynch' and tasks/StatusId eq 300)";
      const actual = buildQuerySql({ filter });
      expect(actual).toEqual(expected);
    });
  ...
  describe('data types', () => {
    ...
    it('should handle GUID values with an in operator', () => {
      ...
      const expected = '?$filter=someProp in (cd5977c2-4a64-42de-b2fc-7fe4707c65cd,cd5977c2-4a64-42de-b2fc-7fe4707c65ce)';
      const actual = buildQuerySql({ filter });
      expect(actual).toEqual(expected);
    });
...
describe('search', () => {
  it('should handle basic search string', () => {
    ...
    const actual = buildQuerySql({ search });
    expect(actual).toEqual(expected);
  });
});
...
describe('transform', () => {
  it('simple aggregation as object', () => {
    ...
    const actual = buildQuerySql({ transform });
    expect(actual).toEqual(expected);
  });
...
describe('select', () => {
  it('should support passing an array of strings', () => {
    ...
    const actual = buildQuerySql({ select });
    expect(actual).toEqual(expected);
  });
...
describe('orderBy', () => {
  it('should support passing an array of strings', () => {
    ...
    const actual = buildQuerySql({ orderBy });
    expect(actual).toEqual(expected);
  });
  ...
  it('should support ordering a nested property within an expand', () => {
    ...
    const actual = buildQuerySql(query);
    expect(actual).toEqual(expected);
  });
...
describe('key', () => {
  it('should support key as simple value', () => {
    const key = 1;
    const expected = '(1)';
    const actual = buildQuerySql({ key });
    expect(actual).toEqual(expected);
  });
  ...
  it('should support key with filter', () => {
    ...
    const actual = buildQuerySql({ key, filter });
    expect(actual).toEqual(expected);
  });
  ...
  it('should support key with expand', () => {
    ...
    const actual = buildQuerySql<Bar>({ key, expand });
    expect(actual).toEqual(expected);
  });
});
...
describe('count', () => {
  it('should support include counts', () => {
    ...
    const actual = buildQuerySql({ count });
    expect(actual).toEqual(expected);
  });
  ...
  it('should allow groupby when querying for only count', () => {
    ...
    const actual = buildQuerySql({ count, transform });
    expect(actual).toEqual(expected);
  });
});
...
describe('pagination', () => {
  it('should support limiting (top)', () => {
    ...
    const actual = buildQuerySql({ top });
    expect(actual).toEqual(expected);
  });
  ...
  it('should support skipping', () => {
    ...
    const actual = buildQuerySql({ skip });
    expect(actual).toEqual(expected);
  });
  ...
  it('should support skipping by token', () => {
    ...
    const actual = buildQuerySql({ skiptoken });
    expect(actual).toEqual(expected);
  });
  ...
  it('should support paginating (skip and limiting)', () => {
    ...
    const actual = buildQuerySql({ top, skip });
    expect(actual).toEqual(expected);
  });
...
describe('expand', () => {
  it('should support basic expansion', () => {
    ...
    const actual = buildQuerySql({ expand });
    expect(actual).toEqual(expected);
  });
  ...
  it('should support multiple expands as an array', () => {
    ...
    const actual = buildQuerySql<Bar>({ expand });
    expect(actual).toEqual(expected);
  });
...
describe('action', () => {
  it('should support an action on a collection', () => {
    const action = 'Test';
    const expected = '/Test';
    const actual = buildQuerySql({ action });
    expect(actual).toEqual(expected);
  });
  ...
  it('should support an action on an entity', () => {
    ...
    const actual = buildQuerySql({ key, action });
    expect(actual).toEqual(expected);
  });
});
...
describe('function', () => {
  it('should support a function on a collection', () => {
    const func = 'Test';
    const expected = '/Test';
    const actual = buildQuerySql({ func });
    expect(actual).toEqual(expected);
  });
  ...
  it('should support a function on an entity', () => {
    ...
    const func = 'Test';
    const expected = '(1)/Test';
    const actual = buildQuerySql({ key, func });
    expect(actual).toEqual(expected);
  });
  ...
  it('should support a function on a collection with an array/collection parameter of a complex type', () => {
    ...
    const expected = '/Test(SomeCollection=@SomeCollection)?@SomeCollection=%5B%7B%22Name%22%3A%22Sean%22%7D%2C%7B%22Name%22%3A%22Jason%22%7D%5D';
    const actual = buildQuerySql({ func });
    expect(actual).toEqual(expected);
  });
});
...
describe('format', () => {
  it('should support format', () => {
    ...
    const actual = buildQuerySql({ format });
    expect(actual).toEqual(expected);
  });
});
