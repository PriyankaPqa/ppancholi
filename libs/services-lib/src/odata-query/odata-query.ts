...
// This code is similar to https://github.com/techniq/odata-query
// Some changes have been made to be used with azure search since it is not compatible with contains, startswith operator
...
export function renderPrimitiveValue(key: string, val: any, aliases: Alias[] = []) {
  // Custom filters for Azure search equivalent of contains
  if(key.includes('/contains_az')) {
    const newKey = key.replace("/contains_az", "");
      ...
      const filtersArray = Object.keys(filter).reduce(
        (result: any[], filterKey) => {
          ...
          if (
            ...
            const op = propName;
            const builtFilters = Object.keys(value).map((valueKey) => buildFilterCore({ [valueKey]: value[valueKey] }));
            if (builtFilters.length) {
              if (op === 'not') {
                result.push(parseNot(builtFilters as string[]));
              } else {
            ...
            if ('type' in value) {
              ...
              operators.forEach((op) => {
                if (COMPARISON_OPERATORS.indexOf(op) !== -1) {
                  ...
                  const collectionClause = buildCollectionClause(filterKey.toLowerCase().replaceAll('/', '_'), value[op], op, propName);
...
function buildAggregate(aggregate: Aggregate | Aggregate[]) {
  ...
  return aggregateArray
    .map((aggregateItem) => (typeof aggregateItem === 'string'
      ? aggregateItem
...
function buildOrderBy<T>(orderBy: OrderBy<T>, prefix = ''): string {
  if (Array.isArray(orderBy)) {
    return (orderBy as OrderByOptions<T>[])
      .map((value) => ((Array.isArray(value) && value.length === 2 && ['asc', 'desc'].indexOf(value[1]) !== -1) ? value.join(' ') : value))
      .map((v) => `${prefix}${v}`).join(',');
...
export default function <T> ({
  ...
  if (filter || typeof count === 'object') params.$filter = buildFilter(typeof count === 'object' ? count : filter, aliases);
  ...
  if (aliases.length > 0) {
    Object.assign(params, aliases.reduce((acc, alias) => Object.assign(acc, { [`@${alias.name}`]: handleValue(alias.value) }),
      {}));
  }

  return buildUrl(path, {
    $select, $search, $skiptoken, $format, ...params,
  });
}
