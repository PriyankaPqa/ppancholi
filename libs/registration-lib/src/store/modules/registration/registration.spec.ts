  ...
  describe('>> Actions', () => {
    describe('fetchEvent', () => {
      it('calls the getEvent service', async () => {
        expect(store.$services.publicApi.fetchRegistrationEvent).toHaveBeenCalledTimes(0);

        await store.dispatch('registration/fetchEvent', { lang: 'en', registrationLink: 'link' });

        expect(store.$services.publicApi.fetchRegistrationEvent).toHaveBeenCalledTimes(1);
      });
