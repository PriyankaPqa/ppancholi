<template>
  <div class="pa-4">
    <rc-data-table
      ...
      <template v-if="$hasLevel(UserRoles.level6)" #headerLeft>
        <rc-add-button-with-menu
          :add-button-label="$t('massActions.authentication-retry.tooltip.add')"
          :items="menuItems"
          data-test="create-mass-authentication-retry"
    ...
    <authentication-retry-case-file-filtering v-if="showProcessByList" :show.sync="showProcessByList" />
  </div>
</template>
...
import AuthenticationRetryCaseFileFiltering from './AuthenticationRetryCaseFileFiltering.vue';

export default mixins(TablePaginationSearchMixin, massActionsTable).extend({
  name: 'AuthenticationRetryHome',

  components: {
    AuthenticationRetryCaseFileFiltering,
    RcDataTable,
    RcAddButtonWithMenu,
    StatusChip,
  },
  ...
  data() {
    return {
      ...
      massActionTypeData: MassActionType.AuthenticationRetry,
      detailsRouteNameData: routes.massActions.authenticationRetry.details.name,
      tableTitleData: 'massAction.authenticationRetryTable.title',
  ...
  computed: {
    ...
    menuItems(): Array<Record<string, string>> {
      return [{
        text: this.$t('massAction.authenticationRetry.table.add.list') as string,
        value: MassActionMode.List,
        icon: 'mdi-filter-variant',
        dataTest: 'add-mass-authentication-retry-via-list',
      }, {
        text: this.$t('massAction.authenticationRetry.table.add.file') as string,
  ...
  methods: {
    goToAdd(item: Record<string, string>) {
      if (item.value === MassActionMode.File) {
        this.$router.push({ name: routes.massActions.authenticationRetry.create.name, query: { mode: MassActionMode.File } });
