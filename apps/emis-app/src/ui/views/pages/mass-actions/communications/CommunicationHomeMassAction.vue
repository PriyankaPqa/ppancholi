<template>
  <div class="pa-4">
    ...
    <communication-case-file-filtering v-if="showProcessByList" :show.sync="showProcessByList" />
  </div>
</template>
...
import CommunicationCaseFileFiltering from './CommunicationCaseFileFiltering.vue';

export default mixins(TablePaginationSearchMixin, massActionsTable).extend({
  name: 'CommunicationHome',

  components: {
    CommunicationCaseFileFiltering,
    RcDataTable,
    RcAddButtonWithMenu,
    StatusChip,
  },
  ...
  methods: {
    goToAdd(item: Record<string, string>) {
      if (item.value === MassActionMode.File) {
        this.$router.push({ name: routes.massActions.communications.create.name, query: { mode: MassActionMode.File } });
