...
import { mockTemporaryAddress, mockOtherData, mockCampgroundData } from '@libs/entities-lib/case-file-individual';
import Component from '../case-file-impacted-individuals/components/ImpactedIndividualAddressTemplate.vue';

const localVue = createLocalVue();

let otherAddress = mockTemporaryAddress({ ...mockOtherData() });
let campgroundAddress = mockTemporaryAddress({ ...mockCampgroundData() });
describe('ImpactedIndividualAddressTemplate.vue', () => {
  ...
  const doMount = async (level = 5) => {
    const options = {
      ...
      data() {
        return {
          updatedAddress: campgroundAddress,
        };
      },
      mocks: {
  ...
  beforeEach(async () => {
    await doMount();
    otherAddress = mockTemporaryAddress({ ...mockOtherData() });
    campgroundAddress = mockTemporaryAddress({ ...mockCampgroundData() });
  });
