...
export const useProvider = (accessToken: string) => {
  const httpClient = getHttpClient({ accessToken, baseUrl: Cypress.env('API_BASE_URL'), useErrorReport: false, localApiPortMap: Cypress.env('API_PORTS'), localBaseUrl: 'https://localhost' });
  return {
    cypress: {
