...
import {
  ...
  createEventAndTeam, createProgramWithTableWithItemAndSubItem, prepareStateMultipleHouseholds,
...
const filePath = 'cypress/downloads/29040MassActionCaseFileStatusUpdate.csv';

describe('[T29040] File can pre-process a case file with the status Open to Closed', { tags: ['@case-file', '@mass-actions'] }, () => {
  describe('Can Roles', () => {
    for (const roleName of filteredCanRoles) {
      describe(`${roleName}`, () => {
        ...
        it('should be able to pre-process a case file with the status Open to Closed', function () {
          ...
          const massCaseFileStatusData = fixtureNewCaseFileStatus({
            ...
            reason: 'Operation ended',
            rationale: 'it is Mandatory field',
          });
          ...
          massCaseFileStatusDetailsPage.getMassActionSuccessfulCaseFiles().should('eq', '1');
          massCaseFileStatusDetailsPage.getNumberFailedRecords().should('eq', '2');
  ...
  describe('Cannot Roles', () => {
    for (const roleName of filteredCannotRoles) {
      describe(`${roleName}`, () => {
        ...
        it('should not be able to pre-process a case file with the status Open to Closed', () => {
